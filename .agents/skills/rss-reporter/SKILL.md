...
使用 webfetch 或浏览器工具抓取 RSS 源/归档页面内容。

**失败重试机制：**

如果 RSS 源获取失败（超时、404、被屏蔽等），**必须按以下顺序重试**：

1. **crw tools 重试** - 使用 `crw_scrape` 直接抓取源 URL，必要时启用 `renderJs=true` 渲染 JavaScript
2. **crw_search / websearch 搜素回退** - 如果 URL 无法直接访问，通过搜索引擎搜索源名称 + 关键词，找到可访问的替代页面
3. **crw_crawl 深度爬取** - 对于归档类页面，使用 `crw_crawl` 爬取站点结构找到最新内容

**注意：不允许因单个源获取失败就跳过该源，必须至少尝试上述两种重试策略。**
